...
> **FAQ-mode note for Tables 1–2.** These tables were generated by `benchmark_eval.py` with the
> FAQ solver in its then-default **structured-Gaussian** multi-start mode (`start_mode="gaussian"`).
> The solver's default has since been changed to **random multi-start** (`start_mode="random"`,
> equal-or-better per the multi-cell ablation in `reports/a5_results.md`), but the canonical
> Tables 1–2 have **not yet been regenerated** under the new default. Re-running
> `benchmark_eval.py` as-committed (which still passes `"gaussian"` explicitly) reproduces these
> numbers; regenerating under the new random default would change the FAQ-arm SWAP counts and is
> a pending follow-up, not represented in these tables.
